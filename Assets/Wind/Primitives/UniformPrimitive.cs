...
public class UniformPrimitive : BasePrimitive
{
    ...
    public override Vector3 GetValue(float p_j, float p_i, float p_k)
    {
        ...
        Vector3 OP = Common.Abs(center - new Vector3(p_i, p_j, p_k));
        if (OP.magnitude > _size)
            return Vector3.zero;

        return Common.Multiply(Vector3.one - OP, direction.normalized);
    }
