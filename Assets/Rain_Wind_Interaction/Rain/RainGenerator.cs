...
public class RainGenerator
{
    ...
    private OBB[] GameObject2OBB(GameObject[] p_obbsGameObject)
    {
        ...
        for (int i = 0; i < oBBs.Length; i++)
        {
            ...
            Vector3 angles = go.GetComponent<CustomRotation>().GetRotation() * Mathf.Deg2Rad; // In radians
            angles = new Vector3(angles.x, -angles.y, -angles.z); // Invert y and z (Unity coordinates system)
